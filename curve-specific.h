...
static void double_jacobian_secp256k1(big * X1,
                                      big * Y1,
                                      big * Z1,
                                      uECC_Curve curve);
static void x_side_secp256k1(big *result, const big *x, uECC_Curve curve);
static void vli_mmod_fast_secp256k1(big *result, big *product);
...
static void double_jacobian_secp256k1(big * X1,
                                      ...
                                      uECC_Curve curve) {
    /* t1 = X, t2 = Y, t3 = Z */
    big t4[num_words_secp256k1];
    big t5[num_words_secp256k1];
    
    if (vbi_is_zero(Z1, num_words_secp256k1)) {
        return;
    }
    ...
    vbi_mod_add(Y1, X1, X1, curve->p, num_words_secp256k1); /* t2 = 2*x1^2 */
    vbi_mod_add(Y1, Y1, X1, curve->p, num_words_secp256k1); /* t2 = 3*x1^2 */
    if (vbi_test_bit(Y1, 0)) {
        big carry = vbi_add(Y1, Y1, curve->p, num_words_secp256k1);
        vbi_rshift1(Y1, num_words_secp256k1);
        Y1[num_words_secp256k1 - 1] |= carry << (WORD_BITS - 1);
    } else {
        vbi_rshift1(Y1, num_words_secp256k1);
    }
    /* t2 = 3/2*(x1^2) = B */
    ...
    vbi_mod_sub(X1, X1, t4, curve->p, num_words_secp256k1); /* t1 = B^2 - A */
    vbi_mod_sub(X1, X1, t4, curve->p, num_words_secp256k1); /* t1 = B^2 - 2A = x3 */
    
    vbi_mod_sub(t4, t4, X1, curve->p, num_words_secp256k1); /* t4 = A - x3 */
    vbi_mod_mul_fast(Y1, Y1, t4, curve);                   /* t2 = B * (A - x3) */
    vbi_mod_sub(Y1, Y1, t5, curve->p, num_words_secp256k1); /* t2 = B * (A - x3) - y1^4 = y3 */
}
...
static void x_side_secp256k1(big *result, const big *x, uECC_Curve curve) {
    ...
    vbi_mod_add(result, result, curve->b, curve->p, num_words_secp256k1); /* r = x^3 + b */
}


static void omega_mult_secp256k1(big *result, const big *right);
static void vli_mmod_fast_secp256k1(big *result, big *product) {
    big tmp[2 * num_words_secp256k1];
    big carry;
    
    vbi_clear(tmp, num_words_secp256k1);
    vbi_clear(tmp + num_words_secp256k1, num_words_secp256k1);
    
    omega_mult_secp256k1(tmp, product + num_words_secp256k1); /* (Rq, q) = q * c */
    
    carry = vbi_add(result, product, tmp, num_words_secp256k1); /* (C, r) = r + q       */
    vbi_clear(product, num_words_secp256k1);
    omega_mult_secp256k1(product, tmp + num_words_secp256k1); /* Rq*c */
    carry += vbi_add(result, result, product, num_words_secp256k1); /* (C1, r) = r + Rq*c */
    
    while (carry > 0) {
        --carry;
        vbi_sub(result, result, curve_secp256k1.p, num_words_secp256k1);
    }
    if (vbi_cmp_unsafe(result, curve_secp256k1.p, num_words_secp256k1) > 0) {
        vbi_sub(result, result, curve_secp256k1.p, num_words_secp256k1);
    }
}
...
static void omega_mult_secp256k1(uint32_t * result, const uint32_t * right) {
    ...
    result[1 + num_words_secp256k1] =
        vbi_add(result + 1, result + 1, right, num_words_secp256k1); 
}
